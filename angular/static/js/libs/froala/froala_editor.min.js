...
!function (a) {
    ...
    b.VALID_NODES = ["P", "PRE", "BLOCKQUOTE", "H1", "H2", "H3", "H4", "H5", "H6", "DIV"], b.COLORS = ["#000000", "#444444", "#666666", "#999999", "#CCCCCC", "#EEEEEE", "#F3F3F3", "#FFFFFF", "#FF0000", "#FF9900", "#FFFF00", "#00FF00", "#00FFFF", "#0000FF", "#9900FF", "#FF00FF", "#F4CCCC", "#FCE5CD", "#FFF2CC", "#D9EAD3", "#D0E0E3", "#CFE2F3", "#D9D2E9", "#EAD1DC", "#EA9999", "#F9CB9C", "#FFE599", "#B6D7A8", "#A2C4C9", "#9FC5E8", "#B4A7D6", "#D5A6BD", "#E06666", "#F6B26B", "#FFD966", "#93C47D", "#76A5AF", "#6FA8DC", "#8E7CC3", "#C27BA0", "#CC0000", "#E69138", "#F1C232", "#6AA84F", "#45818E", "#3D85C6", "#674EA7", "#A64D79", "#990000", "#B45F06", "#BF9000", "#38771D", "#134F5C", "#0B5394", "#351C75", "#741B47", "#660000", "#783F04", "#7F6000", "#274E13", "#0C343D", "#073763", "#201211", "#4C1130"], b.commands = {bold: {title: "Bold (Ctrl + B)", icon: "fa fa-bold"}, italic: {title: "Italic (Ctrl + I)", icon: "fa fa-italic"}, underline: {cmd: "underline", title: "Underline (Ctrl + U)", icon: "fa fa-underline"}, strikeThrough: {title: "Strike Through", icon: "fa fa-strikethrough"}, fontSize: {title: "Font Size", icon: "fa fa-text-height", seed: [
    ...
    ]}, align: {title: "Align", icon: "fa fa-align-center", seed: [
        ...
        var a = navigator.userAgent.toLowerCase(), b = /(chrome)[ \/]([\w.]+)/.exec(a) || /(webkit)[ \/]([\w.]+)/.exec(a) || /(opera)(?:.*version|)[ \/]([\w.]+)/.exec(a) || /(msie) ([\w.]+)/.exec(a) || a.indexOf("compatible") < 0 && /(mozilla)(?:.*? rv:([\w.]+)|)/.exec(a) || [], c = {browser: b[1] || "", version: b[2] || "0"}, d = {};
        ...
        if (this.options.inlineMode) {
            ...
            if (b.left && b.top && b.right && b.bottom) {
                var c = b.left + b.width / 2 + a(window).scrollLeft(), d = b.top + b.height + a(window).scrollTop();
                this.isTouch() && (c = b.left + b.width / 2, d = b.top + b.height), this.showByCoordinates(c, d)
            } else {
                document.execCommand("selectAll");
                var b = this.getRange().getBoundingClientRect(), c = b.left + a(window).scrollLeft(), d = b.top + b.height + a(window).scrollTop();
        ...
        var b = this, c = a("<span>").addClass("f-img-handle").on({movestart: function (c) {
            ...
            {
                var c = a(this), d = b.pageX - parseInt(c.attr("data-start-x"));
        ...
        var b = [], c = ["SPAN", "A", "B", "I", "EM", "STRONG", "STRIKE", "FONT"];
        this.$element.contents().filter(function () {
            if (this.nodeType == Node.TEXT_NODE && a(this).text().trim().length > 0 || c.indexOf(this.tagName) >= 0)b.push(this); else {
                var d = a("<div>");
        ...
        this.isEnabled("insertImage") && (this.drag_support = {filereader: "undefined" != typeof FileReader, formdata: !!window.FormData, progress: "upload"in new XMLHttpRequest})
        ...
        for (var d in this.options.buttons) {
            ...
            if (void 0 != e)switch (e.cmd = this.options.buttons[d], e.cmd) {
                case"color":
                    var f = this.buildDropdownColor(e), g = this.buildDropdownButton(e, "clump-color-picker").append(f);
                    this.$bttn_wrapper.append(g);
                    break;
                case"align":
                    var f = this.buildDropdownAlign(e), g = this.buildDropdownButton(e, "clump-selector").append(f);
                    this.$bttn_wrapper.append(g);
                    break;
                case"fontSize":
                    var f = this.buildDropdownFontsize(e), g = this.buildDropdownButton(e).append(f);
                    this.$bttn_wrapper.append(g);
                    break;
                case"formatBlock":
                    var f = this.buildDropdownFormatblock(e), g = this.buildDropdownButton(e).append(f);
                    this.$bttn_wrapper.append(g);
                    break;
                case"createLink":
        ...
        c = c || "";
        var d = a("<div>").addClass("cl-bttn clump-dropdown").addClass(c), e = a("<button>").addClass("cl-trigger").attr("title", b.title);
        ...
        for (var d in b.seed) {
            var e = b.seed[d], f = a("<div>").append(a("<p>").html(e.title));
        ...
        for (var d in b.seed) {
            var e = b.seed[d], f = a("<li>").append(a("<button>").addClass("cl-bttn").attr("data-cmd", e.cmd).attr("title", e.title).attr("data-activeless", b.activeless).append(a("<i>").addClass(e.icon)));
            c.append(f)
        }
        return c
        ...
        }, this)), e = a('<button type="button" class="f-unlink">').text("UNLINK").on("click touch", a.proxy(function () {
        ...
        if (this.imageMode) {
            var c = new Image;
            return this.options.imageErrorCallback && a.isFunction(this.options.imageErrorCallback) && (c.onerror = this.options.imageErrorCallback({errorCode: 1, errorStatus: "Bad link."})), c.onload = a.proxy(function () {
                this.$element.find(".f-img-editor > img").attr("src", b), this.hide(), this.$progress_bar.hide(), this.$progress_bar.find("span").css("width", "0%").text(""), this.$image_list.show(), this.$image_editor.show(), this.saveUndoStep(), this.callback("replaceImage", {imageURL: b})
            }, this), c.src = b, !1
        }
        var d = this.getSelectionElements()[0], c = new Image;
        this.options.imageErrorCallback && a.isFunction(this.options.imageErrorCallback) && (c.onerror = this.options.imageErrorCallback({errorCode: 1, errorStatus: "Bad link."})), c.onload = a.proxy(function () {
            a(d).prepend('<img alt="Image title" src="' + b + '" width="200" style="float: left; min-width: 16px; min-height: 16px; margin-bottom: ' + this.options.imageMargin + "px; margin-right: " + this.options.imageMargin + 'px">'), a(d).find("img:first").get(0).oncontrolselect = function () {
                return!1
            }, this.saveUndoStep(), this.hide(), this.$progress_bar.hide(), this.$progress_bar.find("span").css("width", "0%").text(""), this.$image_list.show(), this.callback("insertImage", {imageURL: b})
        }, this), c.src = b
        ...
        if (b.length > 0 && this.drag_support.formdata && c.append(this.options.imageUploadParam, b[0]), this.drag_support.formdata) {
            ...
            this.options.crossDomain ? d = this.createCORSRequest("POST", this.options.imageUploadURL) : (d = new XMLHttpRequest, d.open("POST", this.options.imageUploadURL)), d.onload = a.proxy(function () {
                ...
                var b = jQuery.parseJSON(d.responseText);console.log(b);
                "http://uploads.im/api" !== this.options.imageUploadURL ? 
                		200 == b.status_code ? 
                				this.writeImage(b.data.img_url) : this.options.imageErrorCallback({errorCode: 3, errorStatus: "Error during file upload."}) : b.link ? this.writeImage(b.link) : this.options.imageErrorCallback && a.isFunction(this.options.imageErrorCallback) && (img.onerror = this.options.imageErrorCallback({errorCode: 2, errorStatus: "No link in upload response."}))
            }, this),
            d.onerror = a.proxy(function () {
                this.options.imageErrorCallback && a.isFunction(this.options.imageErrorCallback) && (img.onerror = this.options.imageErrorCallback({errorCode: 3, errorStatus: "Error during file upload."})), this.$progress_bar.hide(), this.$progress_bar.find("span").css("width", "0%").text(""), this.$image_list.show()
        ...
        if (this.restoreSelection(), document.execCommand("unlink", !1, b), this.saveSelectionByMarkers(), this.$element.find("span.f-link").each(function (b, c) {
            ...
            this.$element.find("a:empty").remove(), this.callback("insertLink", {URL: b})
        ...
        if (this.undoIndex > 1) {
            ...
            this.$element.html(this.undoStack[--this.undoIndex - 1]), this.hide(), this.callback("undo", {newHTML: this.$element.html(), oldHTML: a})
        }
        this.refreshUndoRedo()
    },b.prototype.redo = function () {
        if (this.undoIndex < this.undoStack.length) {
            var a = this.$element.html();
            this.$element.html(this.undoStack[this.undoIndex++]), this.hide(), this.callback("redo", {newHTML: this.$element.html(), oldHTML: a})
        }
        this.refreshUndoRedo()
    },b.prototype.save = function () {
        this.options.saveURL ? (this.callback("beforeSave"), a.post(this.options.saveURL, {body: this.getHTML()}, a.proxy(function (a) {
        ...
        if (this.isHTML) {
            ...
            this.$element.html(b).attr("contenteditable", !0), this.$box.removeClass("f-html"), this.$editor.find('.cl-bttn:not([data-cmd="html"])').prop("disabled", !1), this.$editor.find("div.cl-bttn").removeAttr("data-disabled"), this.$editor.find('.cl-bttn[data-cmd="html"]').removeClass("active"), this.saveUndoStep(), this.callback("htmlHide", {html: b})
        } else {
            this.isHTML = !0;
            var b = "\n\n" + a.htmlClean(this.$element.html(), {format: !0});
            this.$html_area.val(b).trigger("resize"), this.options.inlineMode && this.$box.find(".html-switch").css("top", this.$box.css("padding-top")), this.$html_area.css("height", this.$element.height() + 20), this.$element.html(this.$html_area).removeAttr("contenteditable"), this.$box.addClass("f-html"), this.$editor.find('button.cl-bttn:not([data-cmd="html"])').prop("disabled", !0), this.$editor.find("div.cl-bttn").attr("data-disabled", !0), this.$editor.find('.cl-bttn[data-cmd="html"]').addClass("active"), this.options.inlineMode && this.hide(), this.callback("htmlShow", {html: b})
        }
        ...
        f.addClass("active"), f.siblings().removeClass("active"), this.callback("backColor", {newColor: b.hexToRGBString(c), oldColor: e})
        ...
        e.addClass("active"), e.siblings().removeClass("active"), this.callback("foreColor", {newColor: b.hexToRGBString(c), oldColor: d})
        ...
        for (var d in c) {
            var e = a(c[d]);
            if ("n" == b)var f = a("<div>").html(e.html()); else var f = a("<" + b + ">").html(e.html());
        ...
        for (var e in d) {
            ...
            if (f.get(0) != this.$element.get(0)) {
                var g = parseInt(f.css("margin-left").replace(/px/, "")), h = Math.max(0, g + c);
                f.css("marginLeft", h)
            } else {
        ...
        if (b instanceof Object)this.options = a.extend({}, this.options, b), this.setBorderColor(), this.setDirection(), this.setDimensions(), this.setSpellcheck(); else {
    ...
    a.fn.editable = function (c) {
        ...
        if (a("html").data("editable") || a("html").on("click", function () {
        }), a("html").data("editable", !0), "string" == typeof c) {
            var f = [];
            return this.each(function () {
                var e = a(this), g = e.data("fa.editable");
    ...
    function c(d, e) {
        var f = [], g = 0 == d.attributes.length, h = 0;
        if (d.tag.isComment)e.allowComments && (f.push("<!--"), f.push(d.tag.rawAttributes), f.push(">"), e.format && b(d, e, f, h - 1)); else {
            ...
            if (!d.isRoot && i && (f.push("<"), f.push(d.tag.name), a.each(d.attributes, function () {
                if (-1 == a.inArray(this.name, e.removeAttrs)) {
                    var b = RegExp(/^(['"]?)(.*?)['"]?$/).exec(this.value), c = b[2], g = b[1] || "'";
            ...
            })), d.tag.isSelfClosing)i && f.push(" />"), g = !1; else if (d.tag.isNonClosing)g = !1; else {
                if (!d.isRoot && i && f.push(">"), h = e.formatIndent++, d.tag.toProtect)n = a.htmlClean.trim(d.children.join("")).replace(/<br>/gi, "\n"), f.push(n), g = 0 == n.length; else {
                    for (var n = [], p = 0; p < d.children.length; p++) {
                        var q = d.children[p], r = a.htmlClean.trim(o(l(q) ? q : q.childrenToString()));
    ...
    a.fn.htmlClean = function (b) {
        ...
        var j, k = /(<(\/)?(\w+:)?([\w]+)([^>]*)>)|<!--(.*?--)>/gi, m = /([\w\-]+)\s*=\s*(".*?"|'.*?'|[^\s>\/]*)/gi, n = new g, o = [n], p = n;
        f.bodyOnly && (j = /<body[^>]*>((\n|.)*)<\/body>/i.exec(b)) && (b = j[1]), b = b.concat("<xxx>");
        for (var q; j = k.exec(b);) {
            var r = j[6] ? new i("--", null, j[6], f) : new i(j[4], j[2], j[5], f), t = b.substring(q, j.index);
            if (t.length > 0) {
            ...
            if (q = k.lastIndex, r.isClosing)d(o, [r.name]) && (o.pop(), p = o[o.length - 1]); else {
    ...
    }, a.htmlClean.defaults = {bodyOnly: !0, allowedTags: [], removeTags: ["basefont", "center", "dir", "frame", "frameset", "iframe", "isindex", "menu", "noframes"], allowedAttributes: [
        ["id"],
        ["class"],
        ["style"],
        ["size"],
        ["color"]
    ], removeAttrs: [], allowedClasses: [], format: !1, formatIndent: 0, replace: [], replaceStyles: [], allowComments: !0, allowEmpty: []}, a.htmlClean.trim = function (b) {
    ...
    var p = ["a", "abbr", "acronym", "address", "b", "big", "br", "button", "caption", "cite", "code", "del", "em", "font", "hr", "i", "input", "img", "ins", "label", "legend", "map", "q", "s", "samp", "select", "option", "param", "small", "span", "strike", "strong", "sub", "sup", "tt", "u", "var"], q = ["address", "button", "caption", "code", "input", "label", "legend", "select", "option", "param"], r = ["h1", "h2", "h3", "h4", "h5", "h6", "p", "th", "td", "object"], s = ["th", "td"], t = [null, "li", ["ul", "ol"], "dt", ["dl"], "dd", ["dl"], "td", ["tr"], "th", ["tr"], "tr", ["table", "thead", "tbody", "tfoot"], "thead", ["table"], "tbody", ["table"], "tfoot", ["table"], "param", ["object"]], u = ["script", "style", "pre", "code"], v = ["area", "base", "br", "col", "command", "embed", "hr", "img", "input", "keygen", "link", "meta", "param", "source", "track", "wbr"], w = ["!doctype", "?xml"], x = [
        ...
        [],
        "a",
        ["accesskey", "class", "href", "name", "title", "rel", "rev", "type", "tabindex"],
        "abbr",
        ["class", "title"],
        "acronym",
        ["class", "title"],
        ...
        "iframe",
        ["class", "height", "name", "sandbox", "seamless", "src", "srcdoc", "width"],
        "input",
        ["accept", "accesskey", "alt", "checked", "class", "disabled", "ismap", "maxlength", "name", "size", "readonly", "src", "tabindex", "type", "usemap", "value"],
        "img",
        ...
        ["class", "colspan", "rowspan"],
        "textarea",
        ["accesskey", "class", "cols", "disabled", "name", "readonly", "rows", "tabindex"],
        "param",
        ["name", "value"],
        "embed",
