...
function parser(node) {
    ...
    if (node.nodeType == 1) {
        ...
        while (attr = directives.shift()) {
            ...
            if(/^ic-(?:href|src|style|class|data)$/.test(name)){
                elm.removeAttr(name);
...
function createRender(root) {
    ...
    var tpl = $(root).html()
        .replace(/&lt;/g, '<')
        .replace(/&gt;/g, '>')
        .replace(/\b(ic-)(?=href|src|style|class|data)/g,'')
        .replace(/&amp;&amp;/g,'&&');
...
directives.add('ic-dialog', function () {
    ...
    var html = "<div style=\"position:fixed; z-index: 1000; left:0; top:0; width:100%; height: 100%; overflow: auto; background: rgba(0,34,89,0.2);\"></div>";
